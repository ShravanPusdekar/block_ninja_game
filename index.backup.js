...
// Bomb configuration
const BOMB_COLOR = { r: 0x00, g: 0x00, b: 0x00 }; // Black bomb
const BOMB_SPAWN_INTERVAL = 15000; // 15 seconds
...
const makeTargetGlueColor = target => {
	if (target.isBomb) return 'rgb(255, 0, 0)'; // Red glue for bomb
...
function resetGame() {
	state.game = {
		...
		cubeCount: 0,
		startTime: Date.now(),
		initialTime: 60, // 60 seconds game time
		remainingTime: 60
	};
	setScore(0);
	setCubeCount(0);
	resetAllTargets();
	lastBombSpawnTime = 0; // Reset bomb spawn timer on game reset
	resetAllCooldowns();
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	// Spawn targets and bombs
	spawnTime -= simTime;
	
	// Spawn bomb every 15 seconds
	const currentTime = Date.now();
	if (currentTime - lastBombSpawnTime > BOMB_SPAWN_INTERVAL) {
		lastBombSpawnTime = currentTime;
		const bomb = getTargetOfStyle(BOMB_COLOR, false);
		bomb.isBomb = true;
		const spawnRadius = Math.min(centerX * 0.8, maxSpawnX);
		bomb.x = (Math.random() * spawnRadius * 2 - spawnRadius);
		bomb.y = centerY + targetHitRadius * 2;
		bomb.z = (Math.random() * targetRadius*2 - targetRadius);
		bomb.xD = Math.random() * (bomb.x * -2 / 120);
		bomb.yD = -20;
		targets.push(bomb);
	}
	...
	for (let i = targets.length - 1; i >= 0; i--) {
		...
		if (target.y > centerY + targetHitRadius * 2) {
			...
			returnTarget(target);
			if (isInGame()) {
				// Always reduce score when a block is missed, regardless of game mode
				incrementScore(-25);
			}
			continue;
		}
		...
		for (let ii=1; ii<=hitTestCount; ii++) {
			...
			if (pointerSpeedScaled > minPointerSpeed) {
				// Check if pointer intersects with target
				const dx = pointerScene.x - target.projected.x;
				const dy = pointerScene.y - target.projected.y;
				const distSq = dx*dx + dy*dy;
				if (distSq < targetHitRadius*targetHitRadius) {
					// Check if it's a bomb
					if (target.isBomb) {
						// End game if bomb is hit
						endGame();
						continue targetLoop;
					}
