...
const RED =     { r: 0xff, g: 0x00, b: 0x00 }; // Red color
const BLACK =   { r: 0x00, g: 0x00, b: 0x00 }; // Black color for the bomb block
const allColors = [BLUE, GREEN, PINK, ORANGE, RED, BLACK]; // All available colors

// Gameplay
let bombBlockSpawned = false;

const spawnBombBlock = () => {
    console.log('=== SPAWNING BOMB BLOCK ===');
    if (bombBlockSpawned) {
        console.log('Bomb block already spawned');
        return;
    }
    
    try {
        console.log('Creating bomb block...');
        
        // Create a new target using the game's target creation system
        const target = getTarget();
        if (!target) {
            console.error('Failed to create bomb block: getTarget returned null');
            return;
        }
        
        console.log('Target created, making it a bomb block...');
        
        // Make it a bomb block
        target.color = BLACK;
        target.wireframe = false;
        
        // Position it in the center of the screen (similar to normal targets)
        const centerX = 0;
        const centerY = 0;
        target.x = centerX;
        target.y = centerY;
        target.z = 0;  // Start at the same depth as other targets
        
        // Make it larger than normal blocks
        target.scaleX = 2;
        target.scaleY = 2;
        target.scaleZ = 2;
        
        // Set initial velocities (similar to normal targets but slower)
        target.xD = 0;  // No horizontal movement
        target.yD = -5; // Move upward like other targets
        target.zD = 0;  // No Z movement (handled by game physics)
        
        // Set properties needed for the game
        target.health = 1;
        target.maxHealth = 1;
        target.points = 0;
        target.alive = true;
        target.isBombBlock = true;
        
        // Force the target to be visible
        target.visible = true;
        
        console.log('Bomb block created:', {
            position: { x: target.x, y: target.y, z: target.z },
            scale: { x: target.scaleX, y: target.scaleY, z: target.scaleZ },
            velocity: { x: target.xD, y: target.yD, z: target.zD },
            inTargets: targets.includes(target)
        });
        
        // The target is already added to the targets array by getTarget()
        ...
        if (typeof requestAnimationFrame === 'function') {
            requestAnimationFrame(() => {
                ...
                // This will force the game to render the new frame
                const event = new Event('mousemove');
                window.dispatchEvent(event);
            });
        }
        ...
        console.error('Error creating bomb block:', error);
    }
};

// Schedule bomb block to appear after 7 seconds
setTimeout(spawnBombBlock, 7000);

const getSpawnDelay = () => {
    console.log('Getting spawn delay...');
...
const slowmoThreshold = 3;  // Spawn bomb block every 3 blocks
...
const state = {
	game: {
		...
		remainingTime: 60, // Start from 60 seconds
...
const getTarget = (() => {
	...
	const slowmoSpawner = makeSpawner({
		chance: 1.0,  // Always spawn when threshold is reached
		cooldownPerSpawn: 0,  // No cooldown between spawns
		maxSpawns: Infinity,  // No limit on number of spawns
	});
	// Reset the spawner when game resets
	state.game.slowmoSpawner = slowmoSpawner;
	...
	return function getTarget() {
		...
		// Check if we should spawn a bomb block (every 3 cubes)
		const shouldSpawnBomb = state.game.cubeCount > 0 && 
		    state.game.cubeCount % 3 === 0 && 
		    (!state.game.lastBombBlockCount || state.game.lastBombBlockCount < state.game.cubeCount);
		
		console.log('Bomb block check:', {
		    cubeCount: state.game.cubeCount,
		    lastBombBlockCount: state.game.lastBombBlockCount,
		    shouldSpawnBomb: shouldSpawnBomb
		});
		
		if (shouldSpawnBomb) {
		    console.log('Creating bomb block at cube count:', state.game.cubeCount);
		    color = RED;  // Make it red to look like a bomb
		    wireframe = false;  // Make it solid
		    health = 1;  // Make it destroyable with one hit
		    isBombBlock = true;  // Mark as bomb block
		    state.game.lastBombBlockCount = state.game.cubeCount;
		}
		...
		target.hit = false;
		target.isBombBlock = isBombBlock;  // Set the bomb block flag
		...
		if (isBombBlock) {
			console.log('Bomb block created:', { 
				...
				isBombBlock: target.isBombBlock 
			});
		}
...
// Red block will be spawned by spawnRedBlock() after 7 seconds

const updateTargetHealth = (target, healthDelta) => {
    // If this is the bomb block, end the game when hit
    if (target.isBombBlock) {
        // Only trigger on actual hits, not just updates
        if (healthDelta < 0) {
            endGame();
            return;
        }
...
function updateTimer() {
	if (isInGame() && state.game.startTime > 0) {
		const now = Date.now();
		const elapsedSeconds = Math.floor((now - state.game.startTime) / 1000);
		state.game.remainingTime = Math.max(0, state.game.initialTime - elapsedSeconds);
		timerNode.textContent = formatTime(state.game.remainingTime);
		
		// End game if time runs out
		if (state.game.remainingTime <= 0) {
			endGame();
		}
	}
}
...
function incrementCubeCount(inc) {
    if (isInGame()) {
        // Initialize cubeCount if it doesn't exist
        state.game.cubeCount = (state.game.cubeCount || 0) + inc;
        
        console.log('Cube count incremented to:', state.game.cubeCount);
        console.log('Next bomb block at:', Math.ceil((state.game.cubeCount + 1) / 3) * 3);
        
        // Update the HUD with the new cube count
        renderScoreHud();
    }
}
...
function playSplatterSound() {
    ...
    if (splatterSound) {
        ...
        splatterSound.volume = 0.5; // 50% volume
        splatterSound.play().catch(e => console.log('Splatter sound play failed:', e));
    }
}
...
function resetGame() {
    ...
    state.game.remainingTime = state.game.initialTime;
    resetAllCooldowns();
    setScore(0);
    setCubeCount(0);
    spawnTime = getSpawnDelay();
    timerNode.textContent = formatTime(state.game.initialTime);
    
    // Reset cube count and ensure it's initialized
    state.game.cubeCount = 0;
    state.game.lastBombBlockCount = 0;  // Reset bomb block counter
    console.log('Game reset, cube count:', state.game.cubeCount);
    
    // Initialize the spawner for bomb blocks
    state.game.slowmoSpawner = {
        shouldSpawn: () => true,  // Always allow spawning
        markSpawned: () => {}     // No-op for this simple spawner
    };
...
// KEYBOARD SHORTCUTS //
////////////////////////
// Pause functionality has been removed
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	// Spawn targets
	spawnTime -= simTime * gameSpeed;  // Scale by gameSpeed
	if (spawnTime <= 0) {
	...
	for (let i = targets.length - 1; i >= 0; i--) {
		...
		if (target.y > centerY + targetHitRadius * 2) {
			targets.splice(i, 1);
			returnTarget(target);
			if (isInGame() && !target.isBombBlock) {
				// Reduce score when a regular block is missed
				incrementScore(-25);
			}
			continue;
		}
			...
			if (distance <= targetHitRadius) {
				...
				if (!target.hit) {
					...
					if (target.isBombBlock) {
						...
						endGame();
						return;
					} else if (pointerSpeedScaled > minPointerSpeed) {
						target.health--;
						incrementScore(10);
						
						// Play splatter sound when block is hit (but not destroyed)
						playSplatterSound();
